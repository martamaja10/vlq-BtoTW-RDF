...
# --- Sample Dictionary ---
sample_dic = samples_trig # This is the name of the list (using list of class objects to keep ordering)
...
outDir='/store/user/jmanagan/BtoTW_Jun2024_trigEffs_elIDs/'
condorDir='/uscms/home/jmanagan/nobackup/BtoTW/rdfjobs_Jun2024_trigEffs_elIDs/' # recommend this be outside git area!
...
if runanalyzer:    
    ...
    os.chdir(relbase)
    print ('tar --exclude="src/.git" --exclude="src/*/.git" --exclude="tmp/" --exclude="src/vlq-BtoTW-SLA" --exclude="src/VLQRDF" --exclude="src/vlq-singleProd-RDF" --exclude="src/vlq-BtoTW-RDF/*.root" --exclude="src/vlq-BtoTW-RDF/*.npz" --exclude="src/vlq-BtoTW-RDF/DeepJetEffs/" --exclude="src/vlq-BtoTW-RDF/PNetEffs" --exclude="src/vlq-BtoTW-RDF/TrigEffs/Jun24*" --exclude=".SCRAM" -zcf '+tarfile+' ./*')
    ...
    for k,v in sample_dic.items(): # This is fine.  It is a dictionary that exists in samples.py that gets imported at runtime
        ...
        num = newNum = 0
        with open('../condor/'+textlist,'r') as rootfiles:
        ...
        jobsPerSample = max(1,round(samplesize/10000000000.))
        ...
        #if "Single" in v.samplename and "APVB" in v.prefix: # trigger list changes every file or so! boo....
        #    jobsPerSample = num
