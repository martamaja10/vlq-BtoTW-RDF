...
bkgProcList = ['wjets200','wjets400','wjets600','wjets800','wjets1200','wjets2500','ttbar','singleT','singleTb']
bkgHistColors = {'ttbar':kAzure+8,
...
for sample in samples.keys():
    ...
    for tag in tags.keys():
        ...
        histo = df_tag.Histo1D((sample+'_'+tag, ';'+plotList[iPlot][-1]+';Events / bin',len(plotList[iPlot][1])-1,array('d',plotList[iPlot][1])),plotList[iPlot][0],"weight")
        histo.Write()  ## is this causing trouble with memory???

        histos[sample+'_'+tag] = histo
...
if doPlotting:
    ...
    for iPtr in histos.keys():
        hists[iPtr] = histos[iPtr].GetValue() ##this is probably the memory hog
